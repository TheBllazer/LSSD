...
const EVENT_TONES = {
  ...
  OWNER_REMOVED: 'var(--warn)',
  IMPOUNDED: 'var(--status-incarcerated)',
  RELEASED: 'var(--ok)',
  SEIZED: 'var(--danger)',
  RETURNED: 'var(--ok)',
};
 ...
 *
 * Générique : citoyens, véhicules et armes partagent la même structure
 * d'événements, seuls les libellés changent. Le journal n'est jamais
 * modifiable — les règles Firestore interdisent la réécriture d'un événement.
 *
 * @param {object} props
 * @param {object[]} props.events
 * @param {boolean} props.loading
 * @param {Record<string, string>} props.eventLabels
 * @param {Record<string, string>} props.fieldLabels
...
export default function HistoryTimeline({
  ...
  eventLabels = {},
  fieldLabels = {},
  emptyMessage = "L'historique se remplit automatiquement à chaque action portée sur cette fiche.",
}) {
  ...
  /** Remplace un nom de champ technique par son libellé métier. */
  const humanize = (label) => {
    if (!label) return '';
    return label.replace(/Champ modifié : (\w+)/, (match, field) =>
      fieldLabels[field] ? `Champ modifié : ${fieldLabels[field]}` : match,
    );
  };
  ...
  if (events.length === 0) {
    return <EmptyState icon={<MdHistory />} title="Aucun événement" message={emptyMessage} />;
  }
